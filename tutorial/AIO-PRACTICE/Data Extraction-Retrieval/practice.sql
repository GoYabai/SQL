-- PostgreSQL version converted for AIO SQL demo.
-- Run this file in pgAdmin Query Tool or psql.
DROP SCHEMA IF EXISTS sql_store CASCADE;
CREATE SCHEMA sql_store;
SET search_path TO sql_store;

-- create-sql-store.sql
-- Standalone setup script for the sql_store database.
...
-- Extra delivered order for Practice #8, so the demo returns both Shipped and Delivered rows.
INSERT INTO orders VALUES (11,2,'2019-04-01',3,'Practice delivered order','2019-04-02',1);
...
-- Extra order item for the additional delivered order.
INSERT INTO order_items VALUES (11,10,1,6.40);
...
CREATE TABLE order_item_notes (
  ...
  PRIMARY KEY (note_id),
  CONSTRAINT fk_order_item_notes_order_items
    FOREIGN KEY (order_id, product_id)
    REFERENCES order_items (order_id, product_id)
    ON UPDATE CASCADE
);

INSERT INTO order_item_notes (note_id, order_id, product_id, note) VALUES (1, 1, 4, 'first note');
...
SET search_path TO sql_hr;

-- create-sql-hr.sql
-- Standalone setup script for the sql_hr database.
...
CREATE SCHEMA sql_invoicing;
SET search_path TO sql_invoicing;

-- create-sql-invoicing.sql
-- Standalone setup script for the sql_invoicing database.
...
CREATE TABLE invoices (
  ...
  payment_total NUMERIC(9,2) NOT NULL DEFAULT 0.00,
...
CREATE SCHEMA sql_inventory;
SET search_path TO sql_inventory;

-- create-sql-inventory.sql
-- Standalone setup script for the sql_inventory database.
CREATE TABLE products (
  product_id INTEGER NOT NULL ,
  name varchar(50) NOT NULL,
  quantity_in_stock INTEGER NOT NULL,
...
-- prac
-- 1.From sql_store.customers in sql_store, find the top 3 customers from VA, FL, or GA whose points are between 300 and 2000.
...
-- 2.From order_items in sql_store, get items in order #6 whose total price is less than 30.
SELECT
  order_id,
  product_id,
  quantity,
...
-- 3.From orders in sql_store, count how many orders each customer has.
SELECT
  customer_id,
...
-- 4.From orders and customers in sql_store, retrieve each order with the customer first name and last name.
SELECT
  order_id,
  c.last_name,
  c.first_name
